...
export function showLoadingOverlay(
  ...
  const overlay = document.createElement('div');
  overlay.className = 'fw-loading-overlay';
  overlay.innerHTML = `
    <div class="fw-spinner fw-spinner--lg"></div>
    <span class="fw-loading-text">${message}</span>
  `;
...
export function setButtonLoading(button: HTMLButtonElement, loading: boolean): void {
  button.classList.toggle('fw-btn--loading', loading);
...
export function showError(
  ...
  const existing = container.querySelector('.fw-error-message');
  if (existing) existing.remove();

  const error = document.createElement('div');
  error.className = 'fw-error-message';
  error.innerHTML = `
    ...
    <span class="fw-error-message__text">${message}</span>
    ${onRetry ? '<button class="fw-error-message__retry">Retry</button>' : ''}
  `;

  if (onRetry) {
    error.querySelector('.fw-error-message__retry')?.addEventListener('click', onRetry);
  }
...
export function clearError(container: HTMLElement): void {
  const error = container.querySelector('.fw-error-message');
